...
export default function colorBlockPlugin(): Plugin {
  return {
    name: 'color-block-plugin',
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (req.url?.match(/^\/color-block\/[a-fA-F0-9]{6}\.png$/)) {
          const match = req.url.match(/\/color-block\/([a-fA-F0-9]{6})\.png$/);
          if (match) {
